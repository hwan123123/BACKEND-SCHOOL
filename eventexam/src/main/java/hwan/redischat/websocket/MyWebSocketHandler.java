package hwan.redischat.websocket;

import hwan.redischat.redis.RedisMessagePublisher;
import hwan.redischat.redis.RedisMessageSubscriber;
...
public class MyWebSocketHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        // Echo 서버는 메시지 보낸 세션에만 다시 메시지를 보내주면 끝!!
        // 그렇다면 chat 서버는??
        String payload = message.getPayload(); // 실제 메시지 (헤더나 메타 정보나 이런 것들을 제외한)
